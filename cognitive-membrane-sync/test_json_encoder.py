...
import numpy as np
from json_encoder_utils import deep_convert, enhanced_json_encoder, safe_json_dump, safe_json_dumps
...
def test_numpy_floats():
    ...
    test_data = {"float16": np.float16(3.14), "float32": np.float32(2.71828), "float64": np.float64(1.41421356)}
...
def test_numpy_boolean():
    ...
    test_data = {"bool_true": np.bool_(True), "bool_false": np.bool_(False)}
...
def test_nested_structures():
    ...
    test_data = {
        "organizations": {
            "cogpilot": {
                "prime": np.int64(2),
                "repositories": [{"complexity": np.float64(4.5), "shape": np.array([3, 3, 2])}],
            },
            "OzCog": {"prime": np.int64(3), "tensor_shape": [np.int32(7), np.int32(2), np.int32(1)]},
        },
...
def test_deep_convert():
    ...
    test_data = {"level1": {"level2": {"level3": {"value": np.int64(42), "array": np.array([1, 2, 3])}}}}
...
def test_enhanced_json_encoder_callback():
    ...
    test_data = {"value": np.int64(123), "array": np.array([1, 2, 3])}
...
def test_file_operations():
    ...
    test_data = {"enterprise": "test", "complexity": np.float64(10.5), "tensor_shape": np.array([7, 3, 10])}
