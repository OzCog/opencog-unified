...
Examples:
  ...
  %(prog)s status
    ...
    init_parser.add_argument("--tensor-dims", default="7,3,10,50,100", help="Tensor dimensions (comma-separated)")
    ...
    # Status command
    subparsers.add_parser("status", help="Show membrane status")
    ...
    try:
        if args.command == "init":
            ...
            bridge.fold_membrane_to_markdown(args.membrane_id)
            ...
            if bridge.membrane_config:
                ...
                total_repos = sum(
                    len(org.get("repositories", {})) for org in bridge.membrane_config.get("organizations", {}).values()
                )
