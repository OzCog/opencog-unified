...
class TestRESTApi:
        ...
        return self.mkatom(
            {
                "type": "ConceptNode",
                "name": "swan",
                "truthvalue": {"type": "simple", "details": {"strength": 0.001, "count": 0.9}},
            }
        )
        ...
        return self.mkatom(
            {
                "type": "ConceptNode",
                "name": "bird",
                "truthvalue": {"type": "simple", "details": {"strength": 0.01, "count": 0.9}},
            }
        )
        ...
        return self.mkatom(
            {
                ...
                "truthvalue": {"type": "simple", "details": {"strength": 1.0, "count": 0.9}},
    ...
    def test_a_post_and_get_node(self):
        ...
        assert (
            TruthValue(
                ...
                count_to_confidence(float(post_result["truthvalue"]["details"]["count"])),
            )
            == atomspace_result.tv
        )
    ...
    def test_b_post_and_get_link(self):
        ...
        swan_animal = self.atomspace.add_link(
            types.InheritanceLink, [self.swan, self.animal], TruthValue(0.5, count_to_confidence(0.4))
        )
    ...
    def test_c_put_and_get_tv_av_node(self):
        ...
        put_response = self.client.put(
            self.uri + "atoms/" + str(jswan["handle"]), data=json.dumps(atom_update), headers=self.headers
        )
        ...
        assert (
            TruthValue(
                ...
                count_to_confidence(float(put_result["truthvalue"]["details"]["count"])),
            )
            == atomspace_result.tv
        )
    ...
    def test_d_put_and_get_tv_av_link(self):
        ...
        atom_update = {"truthvalue": truthvalue, "attentionvalue": attentionvalue}
        put_response = self.client.put(
            self.uri + "atoms/" + str(jatom["handle"]), data=json.dumps(atom_update), headers=self.headers
        )
        put_result = json.loads(put_response.data)["atoms"]
    ...
    def test_l_include_incoming_outgoing(self):
        ...
        get_response = self.client.get(
            self.uri + "atoms?filterby=stirange&stimin=1&" "includeIncoming=true&includeOutgoing=true"
        )
        get_result = json.loads(get_response.data)["result"]["atoms"]
    ...
    def test_m_scheme_command(self):
        ...
        try:
            ...
            post_response = self.client.post(
                self.uri + "scheme", data=json.dumps({"command": "(cog-atomspace)"}), content_type="application/json"
            )
