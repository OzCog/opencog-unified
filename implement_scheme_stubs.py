...
class SchemeStubImplementer:
    ...
    def implement_stub(self, filepath, line_num):
        ...
        try:
            ...
            self.fixes_failed.append(
                {"file": str(filepath.relative_to(self.repo_root)), "line": line_num, "reason": str(e)}
            )
            return False
    ...
    def generate_report(self):
        ...
        return {
            "summary": {"total_implemented": len(self.fixes_applied), "total_failed": len(self.fixes_failed)},
...
if __name__ == "__main__":
    ...
    not_implemented = [
        p
        for p in data["detailed_placeholders"]
        if "not-implemented" in p["content"].lower() or "NotImplementedError" in p["content"]
    ]
