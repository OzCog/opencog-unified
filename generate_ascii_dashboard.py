...
def generate_ascii_dashboard():
    ...
    report.append("│                                                                            │")
    report.append(
        f"│  🔴 Critical (≥0.8): {critical:>4}   {create_bar_chart(critical / total if total > 0 else 0, 30):30}  │"
    )
    report.append(f"│  🟠 High (≥0.6):     {high:>4}   {create_bar_chart(high / total if total > 0 else 0, 30):30}  │")
    report.append(
        f"│  🟡 Medium (≥0.4):   {medium:>4}   {create_bar_chart(medium / total if total > 0 else 0, 30):30}  │"
    )
    ...
    report.append(
        f"│  📋 TODO:   {todo:>5}  {create_bar_chart(todo / total_markers if total_markers > 0 else 0, 40):40}  │"
    )
    report.append(
        f"│  🔧 FIXME:  {fixme:>5}  {create_bar_chart(fixme / total_markers if total_markers > 0 else 0, 40):40}  │"
    )
    report.append(
        f"│  🚧 STUB:   {stub:>5}  {create_bar_chart(stub / total_markers if total_markers > 0 else 0, 40):40}  │"
    )
    report.append("│  ═══════════════                                                           │")
