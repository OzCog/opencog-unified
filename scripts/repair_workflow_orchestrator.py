...
class RepairWorkflowOrchestrator:
    ...
    def step_4_apply_safe_repairs(self, dry_run: bool = True) -> dict:
        ...
        cmd = [sys.executable, str(repair_script), "--repo", str(self.repo_root), "--apply"]
        ...
        try:
            ...
            return {"success": result.returncode == 0, "output": result.stdout, "dry_run": dry_run}
    ...
    def run_workflow(self, apply_safe_repairs: bool = False, dry_run: bool = True) -> dict:
        """
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Orchestrate the complete entelechy repair workflow")
    parser.add_argument("--repo", default=".", help="Repository root path")
    parser.add_argument("--apply-safe", action="store_true", help="Apply safe automated repairs")
    ...
    results = orchestrator.run_workflow(apply_safe_repairs=args.apply_safe, dry_run=not args.no_dry_run)

    # Exit code based on success
    all_success = all(r.get("success", False) or r.get("skipped", False) for r in results.values())
