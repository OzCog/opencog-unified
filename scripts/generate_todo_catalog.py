    ...
    def scan_repository(self):
        ...
        for file_path in self.repo_path.rglob("*"):
            if (
                ...
                and not self._should_skip_path(file_path)
            ):
...
"""
        ...
        for subsystem_name in sorted(by_subsystem.keys()):
            ...
            todos_sorted = sorted(
                todos_in_subsystem, key=lambda x: (self._priority_order(x["priority"]), x["file"], x["line"])
            )
...
*This document is automatically generated and should be updated regularly as the codebase evolves. Last updated: """
