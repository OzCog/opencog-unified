...
class EnhancedMarkerRepair:
    ...
    DOC_CONVERSION_PATTERNS = [
        ...
        (r"(?:TODO|FIXME):?\s*(?:describe|explain)\s+(?:this|what)", "documentation", "Request for explanation"),
    ]
    ...
    def apply_safe_repairs(self, dry_run: bool = True, confidence_threshold: float = 0.85) -> dict:
        ...
        safe_repairs = [
            r for r in self.repairs if not r.requires_review and r.confidence >= confidence_threshold and not r.applied
        ]

        if not safe_repairs:
            return {"applied": 0, "message": "No safe repairs above threshold", "dry_run": dry_run}
    ...
    def export_report(self, output_file: str = "enhanced_repair_report.json") -> dict:
        ...
        report = {
            ...
            "quick_wins": [asdict(r) for r in self.repairs if not r.requires_review and r.confidence >= 0.85],
        }
